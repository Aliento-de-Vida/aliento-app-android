...
import com.alientodevida.alientoapp.domain.entities.network.base.ResponseError.ApiResponseError
...
class HomeFragment : Fragment() {
    ...
    private fun setupObservers(binding: FragmentHomeBinding) {
        ...
        viewModel.onError.observe(owner = viewLifecycleOwner) { onError ->
            onError?.let {
                when(onError.result) {
                    is ApiResponseError<*> -> Toast.makeText(requireContext(), "ApiError", Toast.LENGTH_SHORT).show()
