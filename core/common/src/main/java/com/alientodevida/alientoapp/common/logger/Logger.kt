...
class Logger() {
  ...
  fun v(msg: String, tag: String? = null, tr: Throwable?) {
    val logTag = tag ?: LOG_TAG
    if (logcatEnabled) android.util.Log.v(logTag, msg, tr)
  }
  
  fun d(msg: String, tag: String? = null, tr: Throwable?) {
    val logTag = tag ?: LOG_TAG
    if (logcatEnabled) android.util.Log.d(logTag, msg, tr)
  }
  
  fun i(msg: String, tag: String? = null, tr: Throwable?) {
    val logTag = tag ?: LOG_TAG
    if (logcatEnabled) android.util.Log.i(logTag, msg, tr)
  }
  
  fun w(msg: String, tag: String? = null, tr: Throwable?) {
    val logTag = tag ?: LOG_TAG
  ...
  fun wtf(msg: String, tag: String? = null, tr: Throwable?) {
    val logTag = tag ?: LOG_TAG
